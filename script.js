...
async function loadFromCloud(isSilent = false) {
    ...
    try {
        const res = await fetch("https://alpha-realtime.onrender.com/api/competition-data?t=" + Date.now());
        let serverData = await res.json(); 

        try {
            const supaRes = await fetch(`${SUPABASE_URL}/rest/v1/tournaments?id=neq.-1&select=id,data&t=${Date.now()}`, {
                headers: { 
                    'apikey': SUPABASE_KEY, 
            ...
            let serverVals = Object.values(serverData);
            
            supaData.forEach(row => {
                let match = serverVals.find(s => s.id == row.id || s.db_id == row.id || (s.data && s.data.id == row.id));
                
                if (match) {
                    if (row.data.history) {
                        match.history = row.data.history; 
                        if (!match.data) match.data = {};
                        match.data.history = row.data.history;
                    }
                } else {
                    let newKey = row.data.alphaId || `ALPHA_${row.id}`;
                    serverData[newKey] = { id: row.id, db_id: row.id, ...row.data };
                }
            });
