...
// --- BATCH 2: PITCHFORK FAMILY (ĐÃ FIX TOÁN HỌC) ---
      ...
      // --- BATCH 8: Text Annotation Tools (Hỗ trợ Custom Style qua extendData) ---
      { name: 'plainText', totalStep: 2, needDefaultPointFigure: true, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? (ext || '...') : (ext && ext.text != null ? ext.text : '...'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; return [{ type: 'text', attrs: { x: c[0].x, y: c[0].y, text: txt, align: 'left', baseline: 'top' }, styles: { color: cS.color || '#EAECEF', size: cS.size || 14, family: cS.font || 'Rajdhani, sans-serif', weight: '600' } }]; } },
      { name: 'anchoredText', totalStep: 3, needDefaultPointFigure: true, needDefaultXAxisFigure: true, needDefaultYAxisFigure: true, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? (ext || '...') : (ext && ext.text != null ? ext.text : '...'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var figs = []; if (c.length >= 2) { figs.push({ type: 'line', attrs: { coordinates: [c[0], c[1]] }, styles: { color: 'rgba(0,240,255,0.4)', size: 1, style: 'dashed' } }); } var tx = c.length >= 2 ? c[1].x : c[0].x; var ty = c.length >= 2 ? c[1].y : c[0].y; figs.push({ type: 'text', attrs: { x: tx, y: ty, text: txt, align: 'left', baseline: 'bottom' }, styles: { color: cS.color || '#00F0FF', size: cS.size || 13, family: cS.font || 'Rajdhani, sans-serif', weight: '700' } }); return figs; } },
      { name: 'note', totalStep: 2, needDefaultPointFigure: true, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? (ext || '...') : (ext && ext.text != null ? ext.text : '...'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var x = c[0].x, y = c[0].y; var lines = txt.split('\n'); var lh = 16, pd = 8; var maxLen = lines.reduce(function(m, l) { return Math.max(m, l.length); }, 1); var bw = Math.max(60, maxLen * 7.5 + pd * 2); var bh = lines.length * lh + pd * 2; return [ { type: 'polygon', attrs: { coordinates: [ { x: x, y: y }, { x: x + bw, y: y }, { x: x + bw, y: y + bh }, { x: x, y: y + bh } ]}, styles: { style: 'fill', color: cS.bg || 'rgba(240,185,11,0.15)', borderColor: '#F0B90B', borderSize: 1 }, ignoreEvent: true }, { type: 'text', attrs: { x: x + pd, y: y + pd, text: txt, align: 'left', baseline: 'top' }, styles: { color: cS.color || '#EAECEF', size: cS.size || 12, family: cS.font || 'sans-serif' } } ]; } },
      { name: 'priceNote', totalStep: 2, needDefaultPointFigure: true, needDefaultXAxisFigure: false, needDefaultYAxisFigure: true, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var pts = (ref.overlay && ref.overlay.points) ? ref.overlay.points : []; var priceVal = (pts[0] && pts[0].value != null) ? pts[0].value : null; var dp = (ref.precision && ref.precision.price != null) ? ref.precision.price : 4; var ext = ref.overlay.extendData; var custom = typeof ext === 'string' ? ext : (ext && ext.text != null ? ext.text : ''); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var priceStr = priceVal != null ? priceVal.toFixed(dp) : ''; var label = custom ? (priceStr ? priceStr + '  ' + custom : custom) : (priceStr || '...'); var x = c[0].x, y = c[0].y; var bw = label.length * 7 + 16, bh = 22; return [ { type: 'polygon', attrs: { coordinates: [ { x: x, y: y - bh / 2 }, { x: x + bw, y: y - bh / 2 }, { x: x + bw, y: y + bh / 2 }, { x: x, y: y + bh / 2 } ]}, styles: { style: 'fill', color: cS.bg || 'rgba(14,203,129,0.2)', borderColor: '#0ECB81', borderSize: 1 }, ignoreEvent: true }, { type: 'text', attrs: { x: x + 8, y: y, text: label, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#0ECB81', size: cS.size || 12, family: cS.font || 'Rajdhani, sans-serif', weight: '700' } } ]; } },
      { name: 'pin', totalStep: 2, needDefaultPointFigure: false, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? ext : (ext && ext.text != null ? ext.text : ''); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var x = c[0].x, y = c[0].y, r = 10; var figs = [ { type: 'circle', attrs: { x: x, y: y - r - 10, r: r }, styles: { style: 'fill', color: cS.bg || '#F0B90B', borderColor: '#fff', borderSize: 1.5 } }, { type: 'line', attrs: { coordinates: [{ x: x, y: y - 10 }, { x: x, y: y }] }, styles: { color: cS.bg || '#F0B90B', size: 2 } } ]; if (txt) { figs.push({ type: 'text', attrs: { x: x + r + 4, y: y - r - 10, text: txt, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#EAECEF', size: cS.size || 12, family: cS.font || 'sans-serif' } }); } return figs; } },
      ...
      { name: 'annotation', totalStep: 3, needDefaultPointFigure: true, needDefaultXAxisFigure: true, needDefaultYAxisFigure: true, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? (ext || '...') : (ext && ext.text != null ? ext.text : '...'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var figs = []; if (c.length >= 2) { figs.push({ type: 'line', attrs: { coordinates: [c[0], c[1]] }, styles: { color: '#00F0FF', size: 1 } }); var tx = c[1].x, ty = c[1].y; var bw = txt.length * 7.5 + 16, bh = 22; figs.push({ type: 'polygon', attrs: { coordinates: [ { x: tx, y: ty - bh / 2 }, { x: tx + bw, y: ty - bh / 2 }, { x: tx + bw, y: ty + bh / 2 }, { x: tx, y: ty + bh / 2 } ]}, styles: { style: 'stroke_fill', color: cS.bg || 'rgba(0,240,255,0.1)', borderColor: '#00F0FF', borderSize: 1 }, ignoreEvent: true }); figs.push({ type: 'text', attrs: { x: tx + 8, y: ty, text: txt, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#00F0FF', size: cS.size || 12, family: cS.font || 'Rajdhani, sans-serif', weight: '600' } }); } else { figs.push({ type: 'text', attrs: { x: c[0].x + 8, y: c[0].y, text: txt, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#00F0FF', size: cS.size || 12, family: cS.font || 'Rajdhani, sans-serif', weight: '600' } }); } return figs; } },
      { name: 'comment', totalStep: 2, needDefaultPointFigure: false, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? (ext || '...') : (ext && ext.text != null ? ext.text : '...'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var x = c[0].x, y = c[0].y; var bw = Math.max(80, txt.length * 7 + 20), bh = 30, tail = 8; return [ { type: 'polygon', attrs: { coordinates: [ { x: x, y: y - bh - tail }, { x: x + bw, y: y - bh - tail }, { x: x + bw, y: y - tail }, { x: x + 18, y: y - tail }, { x: x + 10, y: y }, { x: x + 6, y: y - tail }, { x: x, y: y - tail } ]}, styles: { style: 'stroke_fill', color: cS.bg || 'rgba(30,35,42,0.95)', borderColor: '#474d57', borderSize: 1 }, ignoreEvent: true }, { type: 'text', attrs: { x: x + 10, y: y - bh / 2 - tail, text: txt, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#EAECEF', size: cS.size || 12, family: cS.font || 'sans-serif' } } ]; } },
      { name: 'priceLabel', totalStep: 2, needDefaultPointFigure: false, needDefaultXAxisFigure: false, needDefaultYAxisFigure: true, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var pts = (ref.overlay && ref.overlay.points) ? ref.overlay.points : []; var priceVal = (pts[0] && pts[0].value != null) ? pts[0].value : null; var dp = (ref.precision && ref.precision.price != null) ? ref.precision.price : 4; var ext = ref.overlay.extendData; var custom = typeof ext === 'string' ? ext : (ext && ext.text != null ? ext.text : ''); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var label = custom || (priceVal != null ? priceVal.toFixed(dp) : '...'); var x = c[0].x, y = c[0].y, bw = label.length * 7.5 + 20, bh = 20; var arr = 6; return [ { type: 'polygon', attrs: { coordinates: [ { x: x, y: y }, { x: x + arr, y: y - bh / 2 }, { x: x + arr + bw, y: y - bh / 2 }, { x: x + arr + bw, y: y + bh / 2 }, { x: x + arr, y: y + bh / 2 } ]}, styles: { style: 'fill', color: cS.bg || '#F6465D' }, ignoreEvent: true }, { type: 'text', attrs: { x: x + arr + 6, y: y, text: label, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#fff', size: cS.size || 11, family: cS.font || 'Rajdhani, sans-serif', weight: '700' } } ]; } },
      { name: 'signpost', totalStep: 3, needDefaultPointFigure: true, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? (ext || '...') : (ext && ext.text != null ? ext.text : '...'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var tx = c.length >= 2 ? c[1].x : c[0].x; var ty = c.length >= 2 ? c[1].y : c[0].y - 40; var figs = []; if (c.length >= 2) { figs.push({ type: 'line', attrs: { coordinates: [c[0], { x: tx, y: ty }] }, styles: { color: '#848e9c', size: 1, style: 'dashed' } }); } var bw = txt.length * 7.5 + 24, bh = 24, notch = 10; var isRight = tx >= c[0].x; var coords = isRight ? [{ x: tx, y: ty }, { x: tx + notch, y: ty - bh / 2 }, { x: tx + notch + bw, y: ty - bh / 2 }, { x: tx + notch + bw, y: ty + bh / 2 }, { x: tx + notch, y: ty + bh / 2 }] : [{ x: tx, y: ty }, { x: tx - notch, y: ty - bh / 2 }, { x: tx - notch - bw, y: ty - bh / 2 }, { x: tx - notch - bw, y: ty + bh / 2 }, { x: tx - notch, y: ty + bh / 2 }]; figs.push({ type: 'polygon', attrs: { coordinates: coords }, styles: { style: 'fill', color: cS.bg || 'rgba(153,69,255,0.25)', borderColor: '#9945FF', borderSize: 1 }, ignoreEvent: true }); figs.push({ type: 'text', attrs: { x: isRight ? tx + notch + 8 : tx - notch - 8, y: ty, text: txt, align: isRight ? 'left' : 'right', baseline: 'middle' }, styles: { color: cS.color || '#d0aaff', size: cS.size || 12, family: cS.font || 'Rajdhani, sans-serif', weight: '700' } }); return figs; } },
      { name: 'flagMarker', totalStep: 2, needDefaultPointFigure: false, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var txt = typeof ext === 'string' ? ext : (ext && ext.text != null ? ext.text : ''); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var x = c[0].x, y = c[0].y, pw = 3, ph = 30, fw = 22, fh = 14; return [ { type: 'line', attrs: { coordinates: [{ x: x, y: y }, { x: x, y: y - ph }] }, styles: { color: cS.bg || '#F0B90B', size: pw } }, { type: 'polygon', attrs: { coordinates: [ { x: x, y: y - ph }, { x: x + fw, y: y - ph + fh / 2 }, { x: x, y: y - ph + fh } ]}, styles: { style: 'fill', color: cS.bg || '#F0B90B' }, ignoreEvent: true }, ...(txt ? [{ type: 'text', attrs: { x: x + fw + 4, y: y - ph + fh / 2, text: txt, align: 'left', baseline: 'middle' }, styles: { color: cS.color || '#F0B90B', size: cS.size || 11, family: cS.font || 'sans-serif' } }] : []) ]; } },
      ...
      { name: 'insertIcon', totalStep: 2, needDefaultPointFigure: false, needDefaultXAxisFigure: false, needDefaultYAxisFigure: false, createPointFigures: function(ref) { var c = ref.coordinates || []; if (!c.length) return []; var ext = ref.overlay.extendData; var icon = typeof ext === 'string' ? (ext || '★') : (ext && ext.text != null ? ext.text : '★'); var cS = typeof ext === 'object' && ext.styles ? ext.styles : {}; var x = c[0].x, y = c[0].y; return [{ type: 'text', attrs: { x: x, y: y, text: icon, align: 'center', baseline: 'middle' }, styles: { color: cS.color || '#F0B90B', size: cS.size || 22, family: cS.font || 'sans-serif' } }]; } }
    ];
...
// ─────────────────────────────────────────────
  ...
  function openTextEditor(currentText, customStyles, onConfirm) {
    if (USE_NATIVE_PROMPT) {
      var result = window.prompt('Nhập nội dung:', currentText || '');
      if (result !== null) onConfirm(result, customStyles);
      return;
    }
    var existing = document.getElementById('wa-text-editor');
    if (existing) existing.remove();

    // Lấy style được nhúng trong extendData
    var tS = customStyles || {};
    var curColor = tS.color || '#EAECEF';
    var curBg = colorToHex(tS.bg || 'rgba(22, 26, 30, 0.9)'); 
    var curSize = tS.size || 14;
    var curFont = tS.font || 'Rajdhani, sans-serif';
    ...
    function confirm() { 
      ...
      var newStyles = {
        color: document.getElementById('wa-te-color').value,
        bg: hexToRgba(document.getElementById('wa-te-bg').value, 0.8),
        size: parseInt(document.getElementById('wa-te-size').value) || 14,
        font: document.getElementById('wa-te-font').value
      };
      backdrop.remove(); 
      onConfirm(val, newStyles); 
    }
  ...
  function createTextOverlay(chart, toolId, initialData) {
    ...
    function openEditor(extData) {
      // Decode data từ extendData
      var currentText = typeof extData === 'string' ? extData : (extData ? extData.text : '');
      var currentStyles = typeof extData === 'object' && extData ? extData.styles : null;
      ...
      openTextEditor(currentText, currentStyles, function(newText, newStyles) {
        // Đóng gói Text và Styles thành object ném thẳng vào extendData
        if (overlayId) chart.overrideOverlay({ id: overlayId, extendData: { text: newText, styles: newStyles } });
      });
    }
