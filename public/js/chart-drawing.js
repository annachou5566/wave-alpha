...
document.addEventListener('mousedown', function(e) { window.waMouseX = e.clientX; window.waMouseY = e.clientY; });
  ...
  // Cờ (Flag) toàn cục để khóa mọi tương tác của Chart khi Editor đang mở
  window._wa_isEditorActive = false;

  function openTextEditor(currentText, currentStyles, toolId, onConfirm) {
    if (window._wa_isEditorActive) return; // Chặn mở 2 editor cùng lúc
    ...
    var curColor = tStyles.color || '#00F0FF';
    ...
    window._wa_isEditorActive = true;

    // 1. TẠM ẨN CHỮ TRÊN CANVAS (Giữ nguyên viền/nền nếu có)
    var hiddenTextStyles = JSON.parse(JSON.stringify(currentStyles || {}));
    if (!hiddenTextStyles.text) hiddenTextStyles.text = {};
    hiddenTextStyles.text.color = 'rgba(0,0,0,0)'; 
    if (ov) chartObj.overrideOverlay({ id: ov.id, styles: hiddenTextStyles });
    ...
    var ptIndex = (['anchoredText', 'annotation', 'signpost'].includes(ov.name) && liveOv.points && liveOv.points.length > 1) ? 1 : 0;
    var pt = liveOv.points ? liveOv.points[ptIndex] : null;

    var pos = { x: window.innerWidth / 2, y: window.innerHeight / 2 };
    if (pt) {
        var px = chartObj.dataToCoordinate ? chartObj.dataToCoordinate(pt, paneId) : chartObj.convertToPixel(pt, { finder: { paneId: paneId } });
    ...
    var isMiddle = ['priceNote', 'pin', 'annotation', 'comment', 'priceLabel', 'signpost', 'flagMarker'].includes(ov.name);
    var isAlignRight = false;
    if (ov.name === 'signpost' && liveOv.points.length > 1) {
         var p1 = chartObj.dataToCoordinate ? chartObj.dataToCoordinate(liveOv.points[1], paneId) : chartObj.convertToPixel(liveOv.points[1], {finder:{paneId:paneId}});
         var p0 = chartObj.dataToCoordinate ? chartObj.dataToCoordinate(liveOv.points[0], paneId) : chartObj.convertToPixel(liveOv.points[0], {finder:{paneId:paneId}});
    ...
    input.style.cssText = `
      ...
      transform: ${transformCSS}; background: transparent !important; 
      border: none !important; outline: none !important; 
      color: ${curColor} !important; font-family: ${curFont}; 
      font-size: ${curSize}px; line-height: ${exactLineHeight}px;
      ...
      resize: none; z-index: 999999; padding: 0; margin: 0; 
      caret-color: ${curColor}; min-width: 30px; min-height: ${exactLineHeight}px;
    `;
    ...
    // 4. CHẶN TUYỆT ĐỐI EVENT BUBBLING XUỐNG CANVAS KLINECHART
    ...
    input.addEventListener('touchstart', stopEvt, {passive: false});
    input.addEventListener('touchend', stopEvt);
    ...
    // Đồng bộ update text tàng hình (chỉ update data, giữ nguyên style tàng hình)
    input.addEventListener('input', function() { 
        resizeInput(); 
        if (ov && chartObj) chartObj.overrideOverlay({ id: ov.id, extendData: input.value, styles: hiddenTextStyles });
    });
    ...
    function commit() {
        ...
        if (updatedStyles.text) updatedStyles.text.color = curColor; // Khôi phục màu
        
        input.remove();
        
        // Trả cờ active về false sau một khoảng trễ nhỏ để tránh click xuyên qua
        setTimeout(() => { window._wa_isEditorActive = false; }, 100);
        onConfirm(val, updatedStyles);
    }

    input.addEventListener('blur', function() { setTimeout(function() { if (document.activeElement !== input) commit(); }, 50); });
    input.addEventListener('keydown', function(e) {
      if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); commit(); }
      if (e.key === 'Escape') { 
          input.value = currentText || ''; 
          if (ov && chartObj) {
              var restoreStyles = JSON.parse(JSON.stringify(currentStyles || {}));
              if (restoreStyles.text) restoreStyles.text.color = curColor;
              chartObj.overrideOverlay({ id: ov.id, extendData: input.value, styles: restoreStyles });
          }
          commit(); 
      }
    });
  ...
  function createTextOverlay(chart, toolId, initialData) {
    ...
    function doOpenEditor(currentText, currentStyles) {
    ...
    var config = {
      ...
      onDrawEnd: function(event) {
        ...
        if (ov) {
            ...
            window._wa_last_selected_time = Date.now();
            
            doOpenEditor(ov.extendData || '', ov.styles || {});
        }
        return false; // Return false để dừng luồng vẽ của KLineChart
      },
      onSelected: function(event) {
        if (window._wa_isEditorActive) return; // Nếu đang gõ chữ, chặn select mới
        
        isDrawingSessionActive = false;
        var ov = event && event.overlay ? event.overlay : null;
        if (!ov) return;
        
        if (!window.currentSelectedOverlay || window.currentSelectedOverlay.id !== ov.id) {
            window._wa_last_selected_time = Date.now(); 
        }
      ...
      onDeselected: function() {
        if (window._wa_isEditorActive) return; // Đang gõ chữ mà KLineChart đòi deselected thì bỏ qua
        if (typeof hideFloatToolbar === 'function') hideFloatToolbar();
      },
      onClick: function(event) {
        if (window._wa_isEditorActive) return true; // Block event nếu đang mở editor
        ...
        if (cat === 'text') {
            ...
            // Cho phép mở editor nếu click lần 2 sau 200ms
            if (window.currentSelectedOverlay && window.currentSelectedOverlay.id === ov.id && (now - lastTime > 200)) {
                if (typeof hidePanel === 'function') hidePanel();
                if (typeof hideFloatToolbar === 'function') hideFloatToolbar();
                setTimeout(() => doOpenEditor(ov.extendData || '', ov.styles || {}), 10);
                return true; 
            }
        }
        return false;
      ...
      onDoubleClick: function(event) { 
        if (window._wa_isEditorActive) return true;
        var ov = event && event.overlay ? event.overlay : null;
        if (!ov) return true;
        var cat = typeof getToolCategory === 'function' ? getToolCategory(ov.name) : '';
        if (cat === 'text') {
...
var rawExt = overlay._wa_real_text !== undefined ? overlay._wa_real_text : overlay.extendData;
...
function showFloatToolbar(ov, posX, posY) {
  ...
  // 🔥 THÊM GIAO DIỆN THANH TRƯỢT CHO NHÓM CỌ VẼ / DẠ QUANG
  } else if (cat === 'brush') {
    var isHL = ov.name === 'highlighter';
    var lw = ns.line && ns.line.size !== undefined ? ns.line.size : (isHL ? 16 : 3);
    var lo = 1;
    if (ns.line && ns.line.color && ns.line.color.startsWith('rgba')) {
      var m = ns.line.color.match(/rgba\([^,]+,\s*[^,]+,\s*[^,]+,\s*([\d.]+)\)/);
      if (m) lo = parseFloat(m[1]);
    } else if (isHL) lo = 0.45;

    html += fbCpBtn('wa-fb-cp-border', cBorder, svgBorder, 'Màu nét vẽ');
    html += '<div class="wa-fb-sep"></div>';
    
    // Giao diện thanh trượt Size
    html += '<div style="display:flex; align-items:center; gap:6px; padding:0 4px;" title="Độ dày cọ">';
    html += '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8896A7" stroke-width="2"><circle cx="12" cy="12" r="5"/></svg>';
    html += '<input type="range" class="wa-fb-rng" id="wa-fb-size" min="1" max="40" step="1" value="'+lw+'" style="width:60px;" oninput="this.nextElementSibling.innerText=this.value">';
    html += '<span style="color:#60A5FA; font-size:10px; font-family:monospace; min-width:16px; text-align:right;">'+lw+'</span>';
    html += '</div>';

    html += '<div class="wa-fb-sep"></div>';
    
    // Giao diện thanh trượt Opacity
    html += '<div style="display:flex; align-items:center; gap:6px; padding:0 4px;" title="Độ đậm nhạt (Opacity)">';
    html += '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8896A7" stroke-width="2"><path d="M12 22C6.5 22 2 17.5 2 12S6.5 2 12 2s10 4.5 10 10-4.5 10-10 10zM12 2v20"/></svg>';
    html += '<input type="range" class="wa-fb-rng" id="wa-fb-opacity" min="0.05" max="1" step="0.05" value="'+lo+'" style="width:50px;" oninput="this.nextElementSibling.innerText=Math.round(this.value*100)+\'%\'">';
    html += '<span style="color:#60A5FA; font-size:10px; font-family:monospace; min-width:24px; text-align:right;">'+Math.round(lo*100)+'%</span>';
    html += '</div>';
...
// CSS cho thanh trượt kéo trên Floating Toolbar
if (!document.getElementById('wa-fb-rng-style')) {
  var stRng = document.createElement('style');
  stRng.id = 'wa-fb-rng-style';
  stRng.innerHTML = '.wa-fb-rng{-webkit-appearance:none;height:3px;background:#273040;border-radius:2px;outline:none;cursor:pointer;}.wa-fb-rng::-webkit-slider-thumb{-webkit-appearance:none;width:12px;height:12px;border-radius:50%;background:#3B82F6;cursor:pointer;transition:transform 0.1s;}.wa-fb-rng::-webkit-slider-thumb:hover{transform:scale(1.2);}';
  document.head.appendChild(stRng);
}
...
// 🔥 BẮT SỰ KIỆN KÉO THANH TRƯỢT SIZE & OPACITY (CHỈ ÁP DỤNG CHO CỌ VẼ)
if (cat === 'brush') {
  var sizeSlider = bar.querySelector('#wa-fb-size');
  ...
  function updateBrushStyle() {
    ...
    var newSize = sizeSlider ? parseInt(sizeSlider.value) : (objS.line.size || 3);
    var newOp = opSlider ? parseFloat(opSlider.value) : 1;
    var currentColor = objS.line.color || '#3B82F6';
    
    // Tách xuất mã HEX màu để nối với Opacity mới
    var hex = '#3B82F6';
    if (currentColor.startsWith('#')) {
        hex = currentColor.slice(0, 7);
    } else {
        var m = currentColor.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/);
        if (m) hex = '#' + parseInt(m[1]).toString(16).padStart(2,'0') + parseInt(m[2]).toString(16).padStart(2,'0') + parseInt(m[3]).toString(16).padStart(2,'0');
    }
    
    var r = parseInt(hex.slice(1,3), 16)||0, g = parseInt(hex.slice(3,5), 16)||0, b = parseInt(hex.slice(5,7), 16)||0;
    objS.line.color = 'rgba('+r+','+g+','+b+','+newOp+')';
    ...
    if (ov.name === 'highlighter') {
        toolStyles.brush.hlWidth = newSize;
        toolStyles.brush.hlOpacity = newOp;
    } else {
        toolStyles.brush.lineWidth = newSize;
        toolStyles.brush.lineOpacity = newOp;
    }
    if (typeof saveStyles === 'function') saveStyles();
    if (typeof saveAllOverlays === 'function') saveAllOverlays();
  }

  if (sizeSlider) sizeSlider.addEventListener('input', updateBrushStyle);
  if (opSlider) opSlider.addEventListener('input', updateBrushStyle);
  
  // Chặn thao tác kéo thanh trượt bị nhầm thành thao tác kéo di chuyển Toolbar
  if (sizeSlider) { sizeSlider.addEventListener('mousedown', function(e){ e.stopPropagation(); }); sizeSlider.addEventListener('touchstart', function(e){ e.stopPropagation(); }, {passive: false}); }
  ...
  // <--- KẾT THÚC DÁN
    ...
    try {
      ...
      if(tType === 'lines' || tType === 'waves') {
        ...
        let isHL = toolId === 'highlighter';
        config.styles.line = {
          color: typeof hexToRgba === 'function' ? hexToRgba(isHL ? '#FFEB3B' : '#3B82F6', isHL ? 0.45 : 1) : (isHL ? 'rgba(255,235,59,0.45)' : '#3B82F6'),
          size: isHL ? 16 : 3,
          style: 'solid'
        };
