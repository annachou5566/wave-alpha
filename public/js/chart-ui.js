...
// =========================================================================
// 🚀 GIAO DIỆN CÀI ĐẶT PRO: GIỮ NGUYÊN THIẾT KẾ GỐC + ĐẦY ĐỦ LOGIC ĐỘNG
// =========================================================================
window.openChartSettings = function() {
    ...
    menuCfg.querySelectorAll('input[type="color"]').forEach(el => {
        el.addEventListener('input', (e) => {
            ...
            let finalColor = val;
            if (key.includes('FillColor') || key.includes('Fill')) finalColor = engine._dimColor(val, key.includes('hlc') ? c.hlcFillOpacity : c.baselineFillOpacity);
    ...
    if (rngHlcOp) rngHlcOp.addEventListener('input', (e) => {
        engine.update({ 
            hlcFillOpacity: parseFloat(e.target.value),
            hlcUpFillColor: engine._dimColor(c.hlcUpFillColor, parseFloat(e.target.value)),
            hlcDownFillColor: engine._dimColor(c.hlcDownFillColor, parseFloat(e.target.value))
        }, true);
    });
    ...
    if (gridTog) gridTog.addEventListener('click', () => { const isOnoff = gridTog.dataset.on === '1'; engine.update({ gridHorizontal: !isOnoff, gridVertical: !isOnoff }, true); window.openChartSettings(); });
